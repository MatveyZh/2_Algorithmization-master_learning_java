...
class Task_38 {
    ...
    public static void main(String[] args) {
        try (BufferedReader read = new BufferedReader(new InputStreamReader(System.in))) {
            ...
            int [] result = maxDistance(matrix);
            System.out.printf("Maximum distance between of points is %d and %d, is %.2f %n",
                    result[0], result[1], maxDistance);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    private static double distance(int [][] matrix) {
        return Math.sqrt(Math.pow((matrix[0][0] - matrix[1][0]), 2)
                + Math.pow((matrix[0][1] - matrix[1][1]), 2));
    }
    private static int [] maxDistance(int [][] matrix) {
